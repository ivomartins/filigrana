...
(() => {
  ...
  const T = {
    ...
    en: {
      ...
      privacyBody: 'all processing happens locally in your browser; no file is ever sent to any server. No cookies, no accounts, no analytics scripts (visits are counted, in aggregate, by the host). Image metadata (EXIF, location, device) is removed automatically on export. Your preferences (language, settings) stay on this device only — the watermark text is never stored.',
